...
const readOpposingOddsToUser = (allOpposingOdds) => {

  if (!allOpposingOdds.length) {
    console.log('no data given. event type may be invalid')
    return
  }

  console.log('\n')
  allOpposingOdds.forEach((opposingOddsIterable) => {
    const readableSiteNames = opposingOddsIterable.siteNames.join(' & ')
    console.log(`\n${readableSiteNames}:`)
    if(opposingOddsIterable.matchNames.length) {
      opposingOddsIterable.matchNames.forEach((matchname) => {
        const split = matchname.split('_')
        split[1] = 'vs'
...
enter event type: `, async (eventType) => {
  let opposing = true
  const splitEventTypeEntry = eventType.split(' ')
  if (splitEventTypeEntry[1] === 'matching') opposing = false

  const data = await scrapeData(splitEventTypeEntry[0], opposing)

  readOpposingOddsToUser(data)
